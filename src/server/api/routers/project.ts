...
import { z } from "zod";
import { createTRPCRouter, protectedProcedure } from "../trpc"; // 👈 lowercase
...
export const projectRouter = createTRPCRouter({
      ...
      try {
        ...
        if (numOfTuts >= 3000) {
          throw new TRPCError({
            ...
            message:
              "You have reached the maximum number of tutorials for this plan.",
          });
        }
        ...
        if (existingRepo) {
          ...
          const repoName =
            input.githubUrl.split("/").pop() || "Unknown Repository";
        ...
        const response = await generateTutorial(
          input.githubUrl,
          input.accessToken
        );
        ...
        const tutorial = await ctx.db.tutorial.create({
          data: {
            ...
            description:
              validatedTutorial.description ||
              `Comprehensive tutorial for ${repo.name}`,
              ...
              create: validatedTutorial.chapters.map((chapter) => ({
                ...
                subChapters: {
                  create: chapter.content.subChapters.map((subChapter) => ({
                    ...
                    codeSnippets: {
                      create:
                        subChapter.codeSnippets?.map((snippet) => ({
        ...
        revalidateTag(`user-${ctx.user.id}-repos`);
        revalidateTag(`user-${ctx.user.id}-chapters`);

        await processRepositoryEmbeddings(
          repo.githubUrl,
          repo.accessToken,
          tutorial.id
        );
        ...
        return {
          ...
          message: `Tutorial created successfully${
            existingRepo ? " for existing repository" : " with new repository"
          }`,
        };
      } catch (error) {
        ...
        if (error && typeof error === "object" && "code" in error) {
          if (error.code === "P2002") {
            throw new TRPCError({
              code: "CONFLICT",
              message:
                "A repository with this URL already exists for your account",
            });
          }
          if (error.code === "P2025") {
            throw new TRPCError({
              code: "NOT_FOUND",
              message:
                "User account not found. Please refresh the page and try again.",
            });
          }
        }
  ...
  getRepoById: protectedProcedure
    .input(
      z.object({
  ...
  getChapters: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const cachedChapters = await unstable_cache(
        ...
        [`chapters-${input.tutorialId}-${ctx.user.id}`],
        {
          revalidate: 600,
          tags: [
            `tutorial-${input.tutorialId}-chapters`,
            `user-${ctx.user.id}-chapters`,
          ],
        }
      )();
  ...
  getChapter: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const cachedChapter = await unstable_cache(
        ...
        [`chapter-${input.chapterId}-${ctx.user.id}`],
        {
          revalidate: 900,
          tags: [`chapter-${input.chapterId}`, `user-${ctx.user.id}-chapters`],
        }
      )();
  ...
  getLatestTutorials: protectedProcedure
    .input(
      z.object({
        repoId: z.string().min(1, "Repo ID is required"),
      })
